...
```json
{
    ...
    "Items": [/* SharePoint list items - ENDAST user-permitted data */],
    "ItemCount": 15,
...
class SharePointService {
    constructor(authService) {
        ...
        this.apiUrl = 'https://func-sharepoint-prod-001-hmeqadf6h0g9cng8.westeurope-01.azurewebsites.net/api/GetSharePointData';
    }
    ...
    async getSharePointData() {
        try {
            ...
            if (response.status === 403) {
                throw new Error('Du har inte behörighet att komma åt denna SharePoint-data. Kontakta IT-support.');
            }
            ...
            if (!response.ok) {
                ...
                throw new Error(`SharePoint API fel: ${response.status} - ${errorText}`);
            }
            ...
            if (error.message.includes('403')) {
                alert('Du har inte behörighet att komma åt SharePoint-data. Kontakta din IT-administrator.');
...
try {
    ...
    console.log(`Loaded ${result.itemCount} items for user: ${result.userInfo.UserName}`);
